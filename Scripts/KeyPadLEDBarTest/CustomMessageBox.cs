...
namespace KeyPadLEDBarTest
{
    public partial class CustomMessageBox : Form
    {
        ...
        private int _lastValue = -999; // prevents duplicate captures

        public string SelectedButton { get; private set; }

        // Stores ALL key events (press + release) until user clicks Yes
        public List<int> CapturedValues { get; private set; } = new List<int>();
        ...
        public CustomMessageBox(string title, string message, string[] buttons)
        {
            ...
            // Continuous hardware polling timer
            hardwarePollTimer = new System.Windows.Forms.Timer
            {
                Interval = 20 // 50 polls/sec
            };
        ...
        // Ensure timer is shut down on close
        private void CustomMessageBox_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (hardwarePollTimer != null)
            {
        ...
        private void HardwarePollTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (!ReadHardwareKey(out val))
                return;

            // Ignore repeated values (hardware often sends same value repeatedly)
            if (val == _lastValue)
                return;
            ...
            Logger.LogMessage(Level.Info, $"[Key Event Captured] {val}");
            lblMessage.Text = lblMessage.Text + $"\nDetected Key: {val}";
        }

        // Keyboard shortcuts (Y = Yes, N = No, Enter = default, Esc = Cancel)
        private void CustomMessageBox_KeyDown(object sender, KeyEventArgs e)
        {
            string target = null;
        ...
        // Reads hardware key value from device
        private bool ReadHardwareKey(out int val)
        {
            val = -1;
            string resp;
